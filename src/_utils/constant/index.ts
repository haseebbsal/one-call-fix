import { sidebarMenuItemType } from "@/_utils/types";

export const FOOTER_TEXT = {
  DESC: `Connects homeowners with local tradespeople. Hiring a tradeperson has never been so easy!`,
};

export const FOOTER_COLUMNS = [
  {
    title: "About Us",
    links: [
      { text: "About Us", href: "/about-us" },
      // { text: "Contact Us", href: "/contact-us" },
      { text: "Complaints Policy", href: "/complaints-policy" },
      { text: "FAQ", href: "/faq" },
      { text: "Privacy Policy", href: "/privacy-policy" },
    ],
  },
  {
    title: "Homeowners",
    links: [
      { text: "Post a Job", href: "/homeowner/post-a-job" },
      // { text: "How it Works", href: "#" },
      { text: "Contact Us", href: "/contact-us" },

      // { text: "Customer Support", href: "/customer-support" },
      { text: "Request a quote", href: "/homeowner/post-a-job" },
      { text: "Homeowner FAQ", href: "/homeowner-faq" },
    ],
  },
  {
    title: "Tradespeople",
    links: [
      { text: "Sign up ", href: "/tradeperson/signup" },
      { text: "Contact Us", href: "/contact-us" },

      // { text: "How it Works", href: "#" },
      { text: "Refund Policy", href: "/tradesperson-refund-policy" },
      { text: "Code of Conduct", href: "/code-of-conduct" },
      { text: "Tradesperson FAQ", href: "/tradesperson-faq" },
    ],
  },
];

export const TRADESPERSON = [
  {
    title: "Qualified Jobs",
    desc: "Receive High-Quality Jobs using AI, ensuring detailed job scoping.",
  },
  {
    title: "Exclusive Jobs",
    desc: "Improve your chances of conversion by being one of the first 3 tradespeople to submit a quote.",
  },
  {
    title: "Money-back guarantee",
    desc: "Get your money back if you are unable to quote for a job.",
  },
  {
    title: "No Subscriptions / Obligations",
    desc: "Enjoy the flexibility of a contract-free platform. Only pay for leads you choose to pursue, with no hidden fees or subscription obligations.",
  },
];

export const FEATURES = [
  {
    imgSrc: "/icons/quote.png",
    heading: "Receive Quotes Directly Online:",
    desc: "Our AI-Assisted form improves the level of detail provided to tradespeople, increasing your chances of receiving a quote directly online.",
  },
  {
    imgSrc: "/icons/clock.png",
    heading: "Vetted Tradespeople:",
    desc: "We vet all tradespeople before they can submit an offer, saving you the time and effort.  ",
  },
  {
    imgSrc: "/icons/feedback.png",
    heading: "Trusted Feedback:",
    desc: "All reviews are verified as previous customers, helping you to make a confident decision when selecting a tradesperson.",
  },
  {
    imgSrc: "/icons/lock.png",
    heading: "You’re in Control:",
    desc: "Review profiles and shortlist the tradespeople you choose. Your details are only shared with the tradespeople you choose.",
  },
  {
    imgSrc: "/icons/home.png",
    heading: "Streamlined Job Details:",
    desc: "If a home visit is not required, skip the hassle of lengthy conversations. Get all the vital information you need upfront, from deposit requirements to project timelines, without the back-and-forth.",
  },
];

export const REVIEWS = [
  {
    name: "Allie Eilla",
    review:
      "I was hesitant to try pagedone at first, but I'm so glad I did - it's exceeded all of my expectations.",
    avatar: "https://pagedone.io/asset/uploads/1696229994.png",
    rating: 5,
  },
  {
    name: "Allie Eilla",
    review:
      "I was hesitant to try pagedone at first, but I'm so glad I did - it's exceeded all of my expectations.",
    avatar: "https://pagedone.io/asset/uploads/1696229994.png",
    rating: 5,
  },
  {
    name: "Allie Eilla",
    review:
      "I was hesitant to try pagedone at first, but I'm so glad I did - it's exceeded all of my expectations.",
    avatar: "https://pagedone.io/asset/uploads/1696229994.png",
    rating: 5,
  },
];

export const MENU_ITEMS = [
  {
    title: "HOME",
    link: "/",
  },
  {
    title: "POST A JOB",
    link: "/homeowner/post-a-job",
  },
  {
    title: "LOGIN",
    link: "/login",
  },
  {
    title: "TRADESPERSON SIGN UP",
    link: "/tradeperson/signup",
  },
];

export const STEPS = [
  {
    step: "1. Post Your Job",
    desc: "Describe your job using an AI-Assisted form.",
    imgSrc: "/images/post-job.png",
  },
  {
    step: "2. Review Tradespeople Profiles",
    desc: "Review our vetted tradespeople's profiles and offers. We only share your details with those you shortlist!",
    imgSrc: "/images/profile-review.png",
  },
  {
    step: "3. Select Tradesperson",
    desc: "Select the right tradesperson for your job & remember to leave a review!",
    imgSrc: "/images/select-tradesperson.png",
  },
];

export const HOMEOWNER_SERVICE = [
  {
    service:
      "OneCallFix protects your personal details, and only shares it with the tradespeople that you choose.",
  },
  {
    service:
      "With our verified review system, you can trust the feedback you hear on tradespeople.",
  },
  {
    service:
      "With our Al Job Scoping, tradespeople receive more detail which will help you to get quotes directly online.",
  },
  {
    service:
      "You complete one form, and receive multiple quotes. The quotes and tradespeople can then be compared at your leisure since tradespeople will not receive your contact details until you shortlist them.",
  },
  {
    service:
      "We vet all tradespeople, ensuring that they are ID-verified and have provided any regulatory qualifications required. Additionally, we ensure transparency such that you're aware if a tradesperson has provided their public liability insurance.",
  },
];

export const TRADESPEOPLE_SERVICE = [
  {
    service: "We bring the jobs to you - no more effort on digital marketing.",
  },
  { service: "We price on a per lead basis - no contracts or obligations!" },
  {
    service:
      "Get your name in front of leads for free, submit a quote and only pay once you have been shortlisted and have confirmed that you would still like this job.",
  },
  {
    service:
      "Get data insights into how you're performing using our platform - we don't want you to use OneCallFix if you aren't profiting from us, so we provide you with performance data allowing you to understand how profitable you are using our platform.",
  },
];

export const PRIVACY_POLICY = [
  {
    title: "Cons Tetuer Lorem Ipsum",
    desc: [
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
    ],
  },
  {
    title: "Cons Tetuer Lorem Ipsum",
    desc: [
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
    ],
  },
  {
    title: "Cons Tetuer Lorem Ipsum",
    desc: [
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
    ],
  },
];

export const CODE_OF_CONDUCT = [
  {
    title: "Cons Tetuer Lorem Ipsum",
    desc: [
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
    ],
  },
  {
    title: "Cons Tetuer Lorem Ipsum",
    desc: [
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
    ],
  },
  {
    title: "Cons Tetuer Lorem Ipsum",
    desc: [
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
    ],
  },
];

export const COMPLAINTS_POLICY = [
  {
    title: "Cons Tetuer Lorem Ipsum",
    desc: [
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
    ],
  },
  {
    title: "Cons Tetuer Lorem Ipsum",
    desc: [
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
    ],
  },
  {
    title: "Cons Tetuer Lorem Ipsum",
    desc: [
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
      "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuerLorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer",
    ],
  },
];

export const TRADESPERSON_REFUND_POLICY = [
  {
    title: "We understand that sometimes leads may not respond, despite your best efforts to reach out to them.",
    desc: [
      "1. Lead Unresponsiveness:If you are unable to establish contact with a lead within 48 hours of receiving their contact information through our platform, you are eligible for a refund.",
      "2. Refund Request Deadline: All refund requests must be submitted within 7 days of purchasing the lead.",
      "3. Documentation:  You must provide evidence of your attempts to contact the lead, such as screenshots of call logs, emails, or messages sent through our platform.",
      "4. Refund Process: Refund requests will be processed promptly upon review and approval by our team. Refunds will be issued in the form of account credits, which are non-transferable and cannot be exchanged for cash or transferred to bank accounts",
      "5. Limitations: Refunds are subject to limitations. Refunds may not be applicable if the lead has explicitly stated they are no longer seeking services, or if they have already hired another tradesperson.",
      "6. Cancellation of Service: We reserve the right to modify or cancel the refund policy at any time without prior notice. However, any changes will not affect refund requests submitted prior to the effective date of the change.",
      "By adhering to these conditions, we aim to provide a fair and transparent process for handling refund requests and ensuring the satisfaction of our tradespeople members.Best Practices for Contacting Leads",
      'To improve your chances of successfully contacting leads and avoid issues, please follow these best practices'
    ],
  },
  {
    type:"bullet",
    items:[
      {
        bold:"Provide",
        content:"detailed information about contact attempts, including screenshots and timestamps."
      },
      {
        bold:"Contact",
        content:"customers at different times of the day without excessive attempts."
      },
      {
        bold:"Call",
        content:"immediately after receiving the lead’s contact details, then again in the evening or the next morning"
      },
      {
        bold:"Email",
        content:"or text the lead with an introductory message, as leads may not pick up calls from unknown numbers."
      }
    ]
  }
];

export const FAQ = [
  {
    key: "1",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "2",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "3",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "4",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "5",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "6",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "7",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "8",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "9",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "10",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "11",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "12",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "13",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
];

export const HOMEOWNER_FAQ = [
  {
    key: "1",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "2",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "3",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "4",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "5",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "6",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "7",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "8",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "9",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "10",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "11",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "12",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "13",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
];

export const TRADESPERSON_FAQ = [
  {
    key: "1",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "2",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "3",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "4",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "5",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "6",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "7",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "8",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "9",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "10",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "11",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "12",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
  {
    key: "13",
    title: "Tetuer Lorem ipsu Lorem ipsum dolor sit amet, cons tetuer?",
    desc: "Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer Lorem ipsum dolor sit amet, cons tetuer cons tetuer",
  },
];

export const TRADE_PERSON_SIDEBAR_MENU_ITEMS: sidebarMenuItemType[] = [
  {
    label: "Dashboard",
    icon: "/icons/dashboard.png",
    link: "/tradeperson/dashboard",
  },
  {
    label: "Jobs",
    icon: "/icons/jobs.png",
    link:"/tradeperson/jobs"
    // subItems: [
    //   { label: "Pending Jobs", link: "/tradeperson/jobs/pending-jobs" },
    //   { label: "Shortlisted Jobs", link: "/tradeperson/jobs/shortlisted-jobs" },
    //   { label: "My Jobs", link: "/tradeperson/jobs/my-jobs" },
    // ],
  },
  {
    label: "Profile",
    icon: "/icons/profile.png",
    link: "/tradeperson/profile",
  },
  {
    label: "Reviews",
    icon: "/icons/reviews.png",
    link: "/tradeperson/reviews",
  },
  {
    label: "Vetting",
    icon: "/icons/jobs.png",
    subItems: [
      {
        label: "Required Documents",
        link: "/tradeperson/vetting/required-documents",
      },
      {
        label: "Additional Documents",
        link: "/tradeperson/vetting/additional-documents",
      },
    ],
  },
  {
    label: "Account Details",
    icon: "/icons/account-details.png",
    link: "/tradeperson/account-details",
  },
  {
    label: "Billing",
    icon: "/icons/billing.png",
    link: "/tradeperson/billing",
  },
  {
    label: "Refund Requests",
    icon: "/icons/billing.png",
    link: "/tradeperson/refund-requests",
  },
  {
    label: "Feedback",
    icon: "/icons/billing.png",
    link: "/tradeperson/feedback",
  },
  {
    label: "Sign Out",
    icon: "/icons/sign-out.png",
    link: "",
  },
];

export const ADMIN_SIDEBAR_MENU_ITEMS: sidebarMenuItemType[] = [
  {
    label: "Dashboard",
    icon: "/icons/dashboard.png",
    link: "/admin/dashboard",
  },
  {
    label: "Tradespeople Management",
    icon: "/icons/people.png",
    link: "/admin/trade-person-management",
  },
  // {
  //   label: "Jobs Management",
  //   icon: "/icons/service.png",
  //   link: "/admin/jobs-management",
  // },
  {
    label: "Financial Management",
    icon: "/icons/finance.png",
    link: "/admin/financial-management",
  },
  // {
  //   label: "Dispute Resolution",
  //   icon: "/icons/info-circle.png",
  //   link: "/admin/dispute-resolution",
  // },
  // {
  //   label: "Feedback",
  //   icon: "/icons/reviews.png",
  //   link: "/admin/feedback",
  // },
  // {
  //   label: "Verifications Requests",
  //   icon: "/icons/stats.png",
  //   link: "/admin/verification-requests",
  // },
  {
    label: "Refund Requests",
    icon: "/icons/coin.png",
    link: "/admin/refund-requests",
  },
  {
    label: "Sign Out",
    icon: "/icons/sign-out.png",
    link: "",
  },
];

export const DASHBOARD_INPUT_EXTRA_CLASS = {
  input: "max-w-xl placeholder:text-color-14",
  inputWrapper: "!min-h-[48px]",
};


export const DASHBOARD_ITEMS = [
  {
    slug:"leadsPurchased",
    title: "Leads Purchased",
    icon: "/icons/leads.png",
    value: "$150",
    symbol:"none"
  },
  {
    slug:"leadsWon",
    title: "Leads Won",
    icon: "/icons/leads.png",
    value: "1250",
    symbol:"none"
  },
  {
    slug:"winPercentage",
    title: "Win Percentage",
    icon: "/icons/win-percentage.png",
    value: "+5.80%",
    symbol:"percent"
  },
  {
    slug:"totalRevenue",
    title: "Total Revenue",
    icon: "/icons/revenue.png",
    value: "$1,005",
    symbol:"pound"
  },
  {
    slug:"totalSpentOnLeads",
    title: "Total Spent On Leads",
    icon: "/icons/revenue.png",
    value: "90%",
    symbol:"pound"
  },
];



export const DASHBOARD_ITEMS_ADMIN = [
  {
    slug:"tradesPersonSignedUp",
    title: "Tradespeople Signed Up",
    icon: "/icons/leads.png",
    value: "150",
    symbol:"pound"
  },
  {
    slug:"activeTradesPerson",
    title: "Active Tradepeople",
    icon: "/icons/leads.png",
    value: "1250",
    symbol:"none"
  },
  // {
  //   slug:"winPercentage",
  //   title: "Suspended Tradepeople",
  //   icon: "/icons/win-percentage.png",
  //   value: "5.80",
  //   symbol:"percent"
  // },
  {
    slug:"totalJobs",
    title: "Jobs Posted",
    icon: "/icons/revenue.png",
    value: "1,005",
    symbol:"pound"
  },
  {
    slug:"jobsCompleted",
    title: "Jobs Completed",
    icon: "/icons/revenue.png",
    value: "90",
    symbol:"percent"
  },
  {
    slug:"totalRefunds",
    title: "Total Refunds",
    icon: "/icons/revenue.png",
    value: "90",
    symbol:"percent"
  },
];

export const JOB_ITEMS = [
  {
    title: "Repair and Paint Walls and Ceilings",
    location: "18 mins away from PO167GZ",
    description:
      "Due to water ingress we need the plastering sorted in one or two areas, two walls and two ceilings re painted",
    posted: "Posted 3 days ago",
    price: "$15",
    icon: "/icons/jobs-card-icon.png",
  },
  {
    title: "Repair and Paint Walls and Ceilings",
    location: "18 mins away from PO167GZ",
    description:
      "Due to water ingress we need the plastering sorted in one or two areas, two walls and two ceilings re painted",
    posted: "Posted 3 days ago",
    price: "$15",
    icon: "/icons/jobs-card-icon.png",
  },
  {
    title: "Repair and Paint Walls and Ceilings",
    location: "18 mins away from PO167GZ",
    description:
      "Due to water ingress we need the plastering sorted in one or two areas, two walls and two ceilings re painted",
    posted: "Posted 3 days ago",
    price: "$15",
    icon: "/icons/jobs-card-icon.png",
  },
  {
    title: "Repair and Paint Walls and Ceilings",
    location: "18 mins away from PO167GZ",
    description:
      "Due to water ingress we need the plastering sorted in one or two areas, two walls and two ceilings re painted",
    posted: "Posted 3 days ago",
    price: "$15",
    icon: "/icons/jobs-card-icon.png",
  },
];

export const JOB_ITEMS_WITH_SHORTLISTED = [
  {
    title: "Repair and Paint Walls and Ceilings",
    location: "18 mins away from PO167GZ",
    description:
      "Due to water ingress we need the plastering sorted in one or two areas, two walls and two ceilings re painted",
    posted: "Posted 3 days ago",
    price: "$15",
    icon: "/icons/jobs-card-icon.png",
    shortlistedCount: 65,
  },
  {
    title: "Repair and Paint Walls and Ceilings",
    location: "18 mins away from PO167GZ",
    description:
      "Due to water ingress we need the plastering sorted in one or two areas, two walls and two ceilings re painted",
    posted: "Posted 3 days ago",
    price: "$15",
    icon: "/icons/jobs-card-icon.png",
    shortlistedCount: 65,
  },
  {
    title: "Repair and Paint Walls and Ceilings",
    location: "18 mins away from PO167GZ",
    description:
      "Due to water ingress we need the plastering sorted in one or two areas, two walls and two ceilings re painted",
    posted: "Posted 3 days ago",
    price: "$15",
    icon: "/icons/jobs-card-icon.png",
    shortlistedCount: 65,
  },
  {
    title: "Repair and Paint Walls and Ceilings",
    location: "18 mins away from PO167GZ",
    description:
      "Due to water ingress we need the plastering sorted in one or two areas, two walls and two ceilings re painted",
    posted: "Posted 3 days ago",
    price: "$15",
    icon: "/icons/jobs-card-icon.png",
    shortlistedCount: 65,
  },
];

export const JOB_DETAILS = {
  title: "Repair and Paint Walls and Ceilings",
  description: "Lorem ipsum ipsum ipsum ipsum ipsum",
  price: 15,
  contactName: "Mr. John",
  phoneNumber: "075 **** ***",
  email: "jho****@m****.**m",
  details: [
    "Due to water ingress we need the plastering sorted in one or two areas.",
    "Lorem ipsum ipsum ipsum ipsum ipsum",
    "Due to water ingress we need the plastering sorted in one or two areas.",
  ],
  attachments: [
    "/images/attachment-1.png",
    "/images/attachment-2.png",
    "/images/attachment-3.png",
  ],
  location: "Lorem ipsum ipsum ipsum ipsum ipsum",
  reviewLink: "http://reviewlink.com",
};

export const PROFILE_TASKS = [
  { text: "Company Bio", completed: true },
  { text: "Services offered", completed: false },
  { text: "Business Address", completed: true },
  { text: "Upload Profile Photo", completed: false },
  { text: "Upload Previous Jobs", completed: false },
  { text: "Upload proof of ID", completed: false },
  { text: "Upload public liability Insurance", completed: false },
  { text: "Upload any additional documents", completed: false },
];

export const BILLING_ITEMS = [
  {
    title: "Total Balance",
    value: "$25,324.01",
  },
  {
    title: "This Week Earnings",
    value: "$2,388.00",
  },
  {
    title: "Expenses To Date",
    value: "$5,345.00",
  },
];

export const FILTER_ITEMS = [
  {
    id: 1,
    value: "Item 1",
  },
  {
    id: 2,
    value: "Item 2",
  },
  {
    id: 3,
    value: "Item 3",
  },
];


export const HOMEOWNER_AUTH_MENU_ITEMS = [
  {
    title: "POST A JOB",
    link: "/homeowner/post-a-job",
  },
  {
    title: "MY JOB",
    link: "/homeowner/jobs",
  },
  {
    title: "LOGOUT",
    link: "/",
  },
  {
    title: "MY ACCOUNT",
    link: "/homeowner/account-settings",
  },
];